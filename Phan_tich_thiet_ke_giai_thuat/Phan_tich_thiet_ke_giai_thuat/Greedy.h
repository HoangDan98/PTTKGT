...
void prim(int c[8][8], int n)
{
	float lowcost[8];				// mảng trọng số của đỉnh được chọn với các đỉnh còn lại
	int Closest[8];					// giá trị đỉnh kề gần đỉnh th
	int k, min;

	for (int i = 1; i < n; i++)			// Khởi tạo đỉnh đầu tiên trong cây bao trùm, lấy mặc định là đỉnh 1
	{
		lowcost[i] = c[0][i];
		Closest[i] = 0;
	}
